...
export default function AppUpdateChecker() {
  ...
  const checkForUpdates = async () => {
    try {
      ...
      if (ssData?.value && typeof ssData.value === 'object') {
        const val = ssData.value as Record<string, unknown>;
        configData = {
          latest_version_code: typeof val.latest_version_code === 'number' ? val.latest_version_code : 100,
          latest_version_name: typeof val.latest_version_name === 'string' ? val.latest_version_name : '1.0.0',
          is_mandatory: Boolean(val.is_mandatory),
          notes_ar: typeof val.notes_ar === 'string' ? val.notes_ar : undefined,
          notes_fr: typeof val.notes_fr === 'string' ? val.notes_fr : undefined,
          download_url: typeof val.download_url === 'string' ? val.download_url : undefined,
        };
      } else {
        const { data: directData } = await supabase.from('app_config').select('*').maybeSingle();
        if (directData) {
          configData = directData as AppConfig;
        }
      }

      if (configData && configData.latest_version_code > CURRENT_VERSION_CODE) {
        setUpdateAvailable(configData);
      }
    } catch {
