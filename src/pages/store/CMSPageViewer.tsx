...
import { fetchPageBySlug, recordPageView, DEFAULT_STATIC_PAGES } from '../../lib/cms';
  ...
  const loadPage = useCallback(async () => {
    ...
    try {
      ...
      if (fetched) {
        ...
        // Fallback static legal & instruction pages from DEFAULT_STATIC_PAGES
        const s = (slug || '').toLowerCase().trim();
        const foundStatic = DEFAULT_STATIC_PAGES.find(p => p.slug === s || p.key === s || s.includes(p.key.split('-')[0]));
        if (foundStatic) {
          setPage(foundStatic);
        } else {
          setPage(null);
