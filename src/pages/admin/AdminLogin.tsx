    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 text-slate-100" dir={dir}>
      <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 space-y-6 shadow-2xl">
        ...
        <div className="w-full flex flex-col items-center gap-2">
          {isGoogleConfigured ? (
            <div id="googleAdminSignInBtn" className="w-full flex justify-center min-h-[44px]"></div>
          ) : (
            <div className="p-3 bg-slate-950 border border-slate-800 rounded-lg text-center w-full">
              <p className="text-xs text-slate-400">
                {lang === 'ar' ? 'تسجيل الدخول بـ Google غير متاح حالياً' : 'Google Sign-In is unavailable'}
              </p>
            </div>
          )}
        </div>
