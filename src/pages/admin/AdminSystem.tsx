...
export default function AdminSystem() {
  ...
  const loadAllSystemData = async () => {
    ...
    try {
      ...
      // Load OTA release history from Supabase
      const otaData = await getOTAConfig();
      if (otaData) {
        if (otaData.latest_version_name) setLatestVersionName(otaData.latest_version_name);
        if (otaData.latest_version_code) setLatestVersionCode(otaData.latest_version_code);
        if (otaData.release_history && Array.isArray(otaData.release_history)) {
          setReleaseHistory(otaData.release_history);
        }
      }
